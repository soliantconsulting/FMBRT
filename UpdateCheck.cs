...
namespace fmdevtools
{
    class UpdateChecks
    {
        ...
        static string dmtHome = home + Path.DirectorySeparatorChar + ".FMBRT";
        ...
        public static async Task checkForUpdates(string dmtPath) {
            ...
            try {
                if (responseBody == null) {
                    responseBody = await client.GetStringAsync("https://raw.githubusercontent.com/soliantconsulting/FMBRT/master/fmdt.txt");
        ...
        private static void checkVersions(string dmtPath, string versions) {
            ...
            if (new Version(FMBRT.VERSION) < new Version(versionArr[0])) {
            ...
            Regex r = new Regex(@"FMDeveloperTool ([0-9.]*) \(([0-9\-]*)\)", RegexOptions.IgnoreCase);
